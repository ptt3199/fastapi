...
Khá dễ để ngầm hiểu rằng HTTPS là thứ gì đó đơn giản là "được bật" hoặc không.

Tuy nhiên, nó phức tạp hơn như thế nhiều.
/// tip

Nếu bạn đang vội hoặc không quan tâm, hãy tiếp tục với các phần tiếp theo nhằm thực hiện các bước cần thiết để thiết lập mọi thứ với các kỹ thuật khác.
///

Để **tìm hiểu các khái niệm cơ bản về HTTPS**, từ góc độ của người dùng, hãy xem <a href="https://howhttps.works/" class="external-link" target="_blank">https://howhttps.works/</a>.

Bây giờ, từ góc độ của **nhà phát triển**, đây là những điều cần ghi nhớ trong khi nghĩ về HTTPS:

* Đối với HTTPS, **máy chủ** cần **có "chứng chỉ"** được tạo bởi **bên thứ ba**.
    * Những chứng chỉ đó thực sự được **thuê** từ bên thứ ba, không phải là "tạo ra".
* Chứng chỉ có **thời hạn**.
    * Chúng sẽ **hết hạn**.
    * Và sau đó chúng cần được **làm mới**, **thuê lại** từ bên thứ ba.
* Sự mã hóa của kết nối xảy ra ở **cấp TCP**.
    * Đó là một lớp **dưới HTTP**.
    * Do đó, việc xử lý **chứng chỉ và mã hóa** được thực hiện **trước HTTP**.
* **TCP không biết về "miền"**. Chỉ về địa chỉ IP.
    * Thông tin về **miền cụ thể** được đưa vào **dữ liệu HTTP**.
* **Chứng chỉ HTTPS** "chứng thực" một **miền cụ thể**, nhưng giao thức và mã hóa xảy ra ở cấp TCP, **trước khi biết** miền nào đang được xử lý.
* **Mặc định**, điều đó có nghĩa là bạn chỉ có thể có **một chứng chỉ HTTPS cho mỗi địa chỉ IP**.
    * Không quan trọng là máy chủ của bạn lớn thế nào hay mỗi ứng dụng trên nó có nhỏ thế nào.
    * Tuy nhiên, có **một giải pháp** cho điều này.
* Có một **tiện ích mở rộng** cho giao thức **TLS** (giao thức mà xử lí mã hoá ở cấp độ TCP, trước HTTP) được gọi là **<a href="https://en.wikipedia.org/wiki/Server_Name_Indication" class="external-link" target="_blank"><abbr title="Server Name Indication">SNI</abbr></a>**.
